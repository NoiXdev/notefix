...
impl Registry {
    ...
    pub fn remove(&mut self, id: &str) -> Result<ContextEntry, String> {
        if id == self.active_id {
            return Err("cannot remove active context".into());
        }
        if self.contexts.len() <= 1 {
            return Err("cannot remove last context".into());
        }
        let pos = self
            .contexts
            .iter()
...
pub fn load(path: &Path, default_db: &str) -> Registry {
    match std::fs::read_to_string(path)
        .ok()
        .and_then(|j| serde_json::from_str::<Registry>(&j).ok())
    {
...
mod tests {
    ...
    fn flat_context_path_detection() {
        ...
        assert!(is_flat_context_path(
            Path::new("/data/contexts/abc.db"),
            cdir
        ));
        // already migrated (in its own subfolder)
        assert!(!is_flat_context_path(
            Path::new("/data/contexts/abc/notefix.db"),
            cdir
        ));
    ...
    fn add_server_marks_kind_and_url() {
        ...
        let e = r.add_server(
            "s".into(),
            "notes.example".into(),
            "/data/contexts/s/notefix.db".into(),
