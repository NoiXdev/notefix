...
pub fn to_download(referenced: &HashSet<String>, local: &HashSet<String>, server: &HashSet<String>) -> Vec<String> {
    referenced.iter().filter(|p| server.contains(*p) && !local.contains(*p)).cloned().collect()
}

fn base(server_url: &str) -> String { server_url.trim_end_matches('/').to_string() }
...
pub async fn fetch_manifest(server_url: &str, token: &str, workspace_id: &str) -> Result<HashSet<String>, String> {
    let url = format!("{}/api/workspaces/{}/images", base(server_url), workspace_id);
    let resp = client()?.get(&url).bearer_auth(token).header("Accept", "application/json")
        .send().await.map_err(|e| e.to_string())?;
    if !resp.status().is_success() { return Err(format!("manifest HTTP {}", resp.status().as_u16())); }
    let body: Value = resp.json().await.map_err(|e| e.to_string())?;
    Ok(body["data"].as_array().cloned().unwrap_or_default()
        .iter().filter_map(|v| v["path"].as_str().map(str::to_string)).collect())
}

/// POST one image (multipart path + file). `mime` must be the real image mime
/// so the server's `image` validation rule passes.
pub async fn upload_image(server_url: &str, token: &str, workspace_id: &str, path: &str, bytes: Vec<u8>, mime: &str) -> Result<(), String> {
    let url = format!("{}/api/workspaces/{}/images", base(server_url), workspace_id);
    let file_name = path.rsplit('/').next().unwrap_or("image").to_string();
    let part = reqwest::multipart::Part::bytes(bytes).file_name(file_name).mime_str(mime).map_err(|e| e.to_string())?;
    let form = reqwest::multipart::Form::new().text("path", path.to_string()).part("file", part);
    let resp = client()?.post(&url).bearer_auth(token).header("Accept", "application/json")
        .multipart(form).send().await.map_err(|e| e.to_string())?;
    if !resp.status().is_success() { return Err(format!("upload HTTP {}", resp.status().as_u16())); }
    Ok(())
}

/// GET one image's bytes by path.
pub async fn download_image(server_url: &str, token: &str, workspace_id: &str, path: &str) -> Result<Vec<u8>, String> {
    let url = format!("{}/api/workspaces/{}/image", base(server_url), workspace_id);
    let resp = client()?.get(&url).bearer_auth(token).query(&[("path", path)])
        .send().await.map_err(|e| e.to_string())?;
    if !resp.status().is_success() { return Err(format!("download HTTP {}", resp.status().as_u16())); }
