...
export default function VaultConflictDialog({ resolve, onClose }: Props) {
  ...
  const canSubmit = !busy && !!workspacePassphrase && !!localValue.trim();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.45)' }} onClick={onClose}>
      <div className="w-[26rem] max-h-[90vh] overflow-y-auto rounded-lg bg-gray-900 border border-gray-700 p-5" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 mt-2">
          <button onClick={onClose} className="px-3 py-1.5 rounded text-sm text-gray-300 hover:bg-gray-800">{t('vault.cancel')}</button>
          <button
            onClick={() => void submit()}
            disabled={!canSubmit}
