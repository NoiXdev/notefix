    ...
    <div>
      ...
      <SettingsSection title={t('shortcuts.sections.bindings')}>
        {SHORTCUT_ACTIONS.map(a => {
          ...
          return (
            <div key={a.id} className="flex items-center justify-between gap-3 text-sm text-gray-800 border-b border-[var(--line-muted)] pb-1.5">
              <span>{t(a.labelKey)}{conflict && <span className="ml-2 text-xs text-red-600">{t('shortcuts.conflict')}</span>}</span>
              <div className="flex items-center gap-2">
                <kbd className="px-2 py-0.5 rounded bg-white border text-xs" style={{ borderColor: conflict ? '#dc2626' : 'var(--line-muted)' }}>
                  {recording === a.id ? t('shortcuts.pressKey') : comboLabel(combo)}
                </kbd>
                <button onClick={() => setRecording(a.id)} className="px-2 py-0.5 rounded text-xs border" style={{ borderColor: 'var(--line-muted)' }}>{t('shortcuts.change')}</button>
