    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-gray-900 text-gray-200 rounded-lg shadow-xl w-[680px] max-w-[92vw] h-[460px] flex overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="w-56 shrink-0 border-r border-gray-800 overflow-y-auto">
          <div className="px-3 py-2 text-xs font-semibold text-gray-400 border-b border-gray-800">{t('dialogs.history.title')}</div>
          {revisions.length === 0 && <p className="px-3 py-3 text-xs text-gray-500">{t('dialogs.history.noVersions')}</p>}
          {revisions.map(r => (
            <button
              key={r.id}
              aria-label={t('dialogs.history.versionLabel', { id: r.id })}
              onClick={() => pick(r.id)}
