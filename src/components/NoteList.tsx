...
function PinIcon() {
  return (
    <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" className="text-yellow-300 shrink-0 mt-1" aria-hidden>
...
export default function NoteList({
  ...
  const renderRow = (note: Note) => (
    ...
    >
      <div className="flex items-start gap-2">
        {note.pinned ? <PinIcon /> : <div className="w-2 h-2 rounded-sm bg-yellow-300 shrink-0 mt-1.5" />}
  ...
  const sectionHeader = (label: string) => (
    <div className="px-4 pt-3 pb-1 text-gray-600 text-[10px] font-semibold uppercase tracking-widest select-none">
      {label}
    </div>
  );
    ...
    <aside className="w-60 shrink-0 bg-gray-950 flex flex-col h-full select-none">
        ...
        {notes.length === 0 ? (
          ...
          <>
            ...
            {pinned.length > 0 && unpinned.length > 0 && (
              <div className="border-t-2 border-gray-800" data-testid="pin-divider" />
            )}
      ...
      {menu && onTogglePin && (
        <ContextMenu
          ...
          y={menu.y}
          items={[{
            label: menu.note.pinned ? 'Lösen' : 'Anpinnen',
            onClick: () => onTogglePin(menu.note.id, !menu.note.pinned),
          }]}
