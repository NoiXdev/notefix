      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {editMode && (
          <div className="flex items-center gap-1 shrink-0">
            <span className="drag-handle px-1 text-gray-400 cursor-grab" title="Verschieben">⠿</span>
...
export default function Dashboard({ notes, folders, stats, layout, editMode, onSelectNote, onCreateNote, onChangeLayout, onToggleEdit }: Props) {
  ...
  return (
    <div className="h-full overflow-y-auto p-6" style={{ background: '#fef9c3' }}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {editMode && available.map(k => (
            <button key={k} onClick={() => add(k)} className="px-2 py-1 rounded text-xs bg-white border border-gray-300 hover:bg-gray-100">+ {WIDGETS[k].label}</button>
          ))}
      ...
      {active.length === 0 ? (
        <p className="text-sm text-gray-500">Keine Widgets. Klicke „Bearbeiten" und füge welche hinzu.</p>
      ) : (
        <RGL
          className="layout"
          layout={rglLayout}
        ...
        >
          {active.map(w => (
            <div key={w.key}>
              <Card id={w.key} title={WIDGETS[w.key].label} editMode={editMode} onRemove={remove}>
