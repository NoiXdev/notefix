  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.45)' }} onClick={onClose}>
      <div className="w-[26rem] rounded-lg bg-gray-900 border border-gray-700 p-5" onClick={e => e.stopPropagation()}>
        <h2 className="text-gray-100 text-base font-semibold mb-2">Systemprüfung</h2>
        <p className="text-gray-400 text-sm mb-3">Einige Rechte fehlen oder ein Ordner ist nicht beschreibbar:</p>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button onClick={onOpenSettings} className="px-3 py-1.5 rounded text-sm font-medium" style={{ background: '#fde047', color: '#1c1917' }}>Einstellungen öffnen</button>
        </div>
      </div>
    </div>
  );
